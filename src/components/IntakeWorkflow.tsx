...
const STEPS: Step[] = [
  {
    ...
    summary:
      "On intake, have a conversation with the patient about any animals at home.",
    items: [
  ...
  },
  {
    ...
    link: { href: "/resources?category=humane-aco", label: "Open Humane Societies & ACOs" },
  },
  {
    n: "3",
    ...
    link: { href: "/resources?category=food-pantry", label: "Open Pet Food Pantries" },
  },
];
...
export function IntakeWorkflow() {
  ...
  return (
    <div>
      <div className="no-print sticky top-[57px] z-30 -mx-4 mb-8 border-b border-edge bg-background/95 px-4 py-3 backdrop-blur sm:mx-0 sm:rounded-xl sm:border sm:px-5">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setDone({})}
              className="rounded-md border border-edge px-3 py-1.5 text-sm font-semibold text-ink hover:border-accent hover:text-accent"
            >
              Reset
            </button>
            <button
        ...
        {STEPS.map((step) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="grid h-9 w-9 place-items-center rounded-full bg-accent-soft font-serif text-base font-bold text-accent">
                {step.n}
              </span>
              <h2 className="font-serif text-xl font-bold text-ink-strong">
                {step.title}
              </h2>
            </div>
            ...
            {step.link && (
              <Link
                href={step.link.href}
                className="no-print mt-4 inline-block text-sm font-semibold text-accent hover:text-accent-hover"
              >
                {step.link.label} →
              </Link>
            )}
      ...
      <div className="mt-8 rounded-xl border border-accent-ring bg-accent-soft p-5">
        <h3 className="font-serif text-lg font-bold text-ink-strong">
